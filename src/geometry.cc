...
G4VPhysicalVolume *MyGeometry::Construct()
{
	...
	solidFlux = new G4Sphere("solidFlux", 0., rPhantom*1.00001, 0, 360*deg, 0, 180*deg);
		
	logicFluxSphere = new G4LogicalVolume(solidFlux, materials->GetMaterial("G4_AIR"), "logicFluxSphere");
	physFluxSphere = new G4PVPlacement(0, G4ThreeVector(0.,0.*m,rPhantom), logicFluxSphere, "physFlux", logicAir, false, 0, checkOverLaps);
